...
3. 核对当前数据：
   ...
   - 能查到时补充财务和估值背景
4. 选择工具：
   - A 股/ETF：优先使用 Codex 原生 `myhhub-stock` 量化证据 + `daily-stock-analysis` 日常看板
   - 美股/港股/跨市场：优先使用 Codex 原生 `tradingagents` 多视角流程 + 实时公开信息核验
   - 高置信度或复杂判断：组合多个可用的专业能力和独立数据源
   - 三套专业 skill 都由当前 Codex 执行；其角色视角、看板标签和派生指标不是独立外部证据
   - 底层公开来源、可复算指标与 wiki 经验互相冲突时，必须在决策记录里说明如何处理
5. 定性评估证据：
   - 市场环境
   - 流动性
   - 趋势与量价
