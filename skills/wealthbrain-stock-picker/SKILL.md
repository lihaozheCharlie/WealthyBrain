...
description: "用于 WealthBrain 的选股和推荐决策：筛选股票、分析单只股票或股票池、给出买入/持有/减仓/卖出/观察/回避等研究建议，并结合专业分析 skill、实时公开信息和本地 wiki 知识记录决策。不要用于复盘历史推荐或计算收益；这些任务使用 wealthbrain-stock-reflector。"
---

# WealthBrain Stock Picker

这个 skill 负责“做新的选股决策”。它把专业分析 skill、当前公开信息和 WealthBrain wiki 记忆合在一起，形成可复盘、可追踪的推荐记录。
...
- 选择证据来源、综合专业分析 skill/公开信息/wiki 经验：读取 `references/evidence_sources.md`。
...
按需使用这些本地 skill/wrapper：

- `external-skills/tradingagents/SKILL.md`：当前 Codex 原生执行。
- `external-skills/daily-stock-analysis/SKILL.md`：当前 Codex 原生执行。
- `external-skills/myhhub-stock/SKILL.md`：当前 Codex 原生执行；有 OHLCV CSV 时可调用本地标准库脚本。
